...
// note: add a 1uf titanian cap and a 10uf cap
...
#define BACKLIT_VALUE 200   //display brightness (0-255, inversed), 200
...
void display_info()
{
    ...
    sprintf(lcd_buffer, "%4ld|T%4d|I%4ld", v_b3, temp_0, c_usb_1);
...
void button_low_isr()
{
    ...
    //if lcd backlight is off, turn on the backlight first
    if(back_led >= 255)
    {
        back_led = BACKLIT_VALUE;
        analogWrite(back_led_pin, back_led);
    }
    else //otherwise toggle the led light
    {
        led = !led;
